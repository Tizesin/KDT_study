...
_.last = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  if(n === undefined)
    return [arr[arr.length-1]]; //_.slice(arr,arr.length-1,arr.length);
  else if(n === 0)
    return [];
  else if(n > arr.length-1)
    return arr;
  return _.drop(arr,n-1,arr.length);
...
_.uniq = function (arr) {
  let result = [];
  // _.each(arr,function(iteratee,index) {
  //   result.push(iteratee);
...
_.map = function (arr, iteratee) {
  ...
  let result = [];
  _.each(arr,function(ele,idx,array) {
    result.push(iteratee(ele,idx,array)); 
  });
  return result;
};
...
_.pluck = function (arr, keyOrIdx) {
  ...
  _.map(arr,function(val){
    result.push(val[keyOrIdx]);
  });

  //let result = _.map(arr,function(val) => val[keyOrIdx])
  return result;
};
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  _.each(arr,function(el,idx,src){
    if(acc === undefined) acc = el;
    else acc = iteratee(acc,el,idx,src);
  })
  return acc;
};
