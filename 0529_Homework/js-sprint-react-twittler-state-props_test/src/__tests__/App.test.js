...
describe("App.js React Router 컴포넌트 적용", () => {
  test('Route path가 "/" 인 Tweets 컴포넌트가 있어야 합니다.', () => {
    ...
    expect(hasPath[0].props.element.type).toBe(Tweets);
  });
  ...
  test('Route path가 "/about" 인 About 컴포넌트가 있어야 합니다.', () => {
    ...
    expect(hasPath[0].props.element.type).toBe(About);
  });
  ...
  test('Route path가 "/mypage" 인 MyPage 컴포넌트가 있어야 합니다.', () => {
    ...
    expect(hasPath[0].props.element.type).toBe(MyPage);
  });
  ...
  test('Tweets 아이콘의 Link 컴포넌트는 "/" 로 연결되어야 합니다.', () => {
    ...
    expect(
      linkInstanceWithRootPath
        .findByType("i")
        .props.className.includes("far fa-comment-dots")
    ).toBeTruthy();
  });
  ...
  test('About 아이콘의 Link 컴포넌트는 "/about" 로 연결되어야 합니다.', () => {
    ...
    expect(
      linkInstanceWithSidebarPath
        .findByType("i")
        .props.className.includes("far fa-question-circle")
    ).toBe(true);
  });
  ...
  test('MyPage 아이콘의 Link 컴포넌트는 "/mypage" 로 연결되어야 합니다.', () => {
    ...
    expect(
      linkInstanceWithMyPageLink
        .findByType("i")
        .props.className.includes("far fa-user")
    ).toBe(true);
  });
});
...
describe("React Router로 SPA 구현하기", () => {
  ...
  test('처음 접속 시, URL path가 "/" 이여야 합니다.', async () => {
    ...
    const hasHome = appInstance
      .findByType(Routes)
      .props.children.find((child) => child.props.path === rootPath);

    expect(hasHome.props.path).toBe(rootPath);
  ...
  test("About 메뉴를 누르면 URL path가 /about으로 라우트 되어야 합니다.", async () => {
    ...
    const hasAbout = appInstance
      .findByType(Routes)
      .props.children.find((child) => child.props.path === aboutPath);

    expect(hasAbout.props.path).toBe(aboutPath);
  ...
  test("MyPage 메뉴를 누르면 URL path가 /mypage로 라우트 되어야 합니다.", async () => {
    ...
    const appInstance = TestRenderer.create(<App />).root;
    const hasMyPage = appInstance
      .findByType(Routes)
      .props.children.find((child) => child.props.path === myPagePath);

    expect(hasMyPage.props.path).toBe(myPagePath);
...
describe("Tweets.js 트윗 전송 폼 테스트", () => {
  ...
  test('유저 이름을 작성할 수 있는 input 엘리먼트가 있어야 합니다. (className : "tweetForm__input--username")', () => {
    ...
    const input = textboxes.find(
      (textbox) =>
        ...
        textbox.classList.contains("tweetForm__input--username")
    );
  ...
  test('트윗을 작성할 수 있는 textarea 엘리먼트가 있어야 합니다. (className : "tweetForm__input--message")', () => {
    ...
    const textarea = textboxes.find(
      (textbox) =>
        ...
        textbox.classList.contains("tweetForm__input--message")
    );
  ...
  test("<button>의 값이 변경될 때 onClick 이벤트 핸들러가 불려야 합니다.", () => {
    ...
    console.log(buttons[0].props.className);
    console.log("tweetForm__submitButton");

    const { node, props } = buttons.find(
      (button) => button.props.className === "tweetForm__submitButton"
    );
  ...
  describe("유저 이름과 트윗을 작성하고, 트윗 버튼을 누르면 새로운 트윗이 추가되어야 합니다.", () => {
    ...
    test("새로 추가된 트윗을 포함하여 보여줘야 합니다.", () => {
      const { queryByText, container, queryAllByRole } = render(<Tweets />);
